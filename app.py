...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        num_nessages, words, num_media_messages, num_links = helper.fetch_stats(selected_user,df)
        
        col1, col2, col3, col4 = st.columns(4)

        with col1:
            st.header("Total Analysis")
            st.title(num_nessages)
        with col2:
        ...
        fig,ax = plt.subplots(figsize=(75, 28))
        ax = sns.heatmap(user_heatmap, annot=True, cmap='viridis')
        plt.xlabel('Period (Hour of Day)')
